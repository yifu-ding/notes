...
> [!NOTE] 说明
> 下表按主线版本梳理；`新 feature` 仅记录相对前代最具代表性的能力／架构变化，`局限` 为该代模型的典型边界，并非穷尽所有问题。

### LLaMA 版本速览

| 版本 | 发布时间 | 参数规模／代表规格 | 新 feature | 局限 |
| --- | --- | --- | --- | --- |
| **LLaMA-1** | 2023.02 | Dense：7B / 13B / 33B / 65B | 以公开数据训练的高效基础模型；采用 *Pre-Norm + RMSNorm + SwiGLU + RoPE* | 仅基础文本模型；上下文较短，未针对对话、工具或多模态进行原生优化 |
| **LLaMA-2** | 2023.07 | Dense：7B / 13B / 70B | 开放商用许可；推出 **Llama 2-Chat**，强化对话对齐与安全性 | 上下文仍主要为 4K；文本单模态，复杂推理与工具能力有限 |
| **LLaMA-3** | 2024.04 | Dense：8B / 70B | 新 tokenizer、GQA 与更强预训练／后训练；提升推理、代码和指令跟随 | 首发仍是文本模型，8K 上下文；多语、多模态与更长上下文留待后续版本 |
| **LLaMA-3.1** | 2024.07 | Dense：8B / 70B / 405B | **128K 上下文**；增强多语、工具调用与 agent 配套能力 | 仍为文本模型；405B 的部署显存与推理成本很高 |
| **LLaMA-3.2** | 2024.09 | 文本：1B / 3B；视觉：11B / 90B | 首批视觉语言模型；小尺寸文本模型面向端侧／移动部署 | 多模态仅覆盖部分视觉规格；小模型的复杂推理能力受参数规模限制 |
| **LLaMA-3.3** | 2024.12 | Dense：70B | 以较低服务成本取得接近 LLaMA-3.1 405B 的文本性能 | 仍是文本模型；没有引入新的原生多模态或超长上下文范式 |
| **LLaMA-4** | 2025.04 | MoE：Scout 109B、Maverick 400B | 原生多模态 **MoE**；长上下文与专家混合架构进一步扩展 | MoE 部署与路由更复杂；许可证、硬件门槛和模型可用性仍限制部分场景 |

### Qwen 版本速览

| 版本 | 发布时间 | 参数规模／代表规格 | 新 feature | 局限 |
| --- | --- | --- | --- | --- |
| **Qwen1** | 2023.09 | Dense：1.8B / 7B / 14B / 72B | 中文与英文能力并重；建立通用、代码与对话模型产品线 | 早期上下文、对齐与多语覆盖仍有限；工具／推理能力不如后续版本成熟 |
| **Qwen1.5** | 2024.02 | 0.5B–110B，并含 MoE 变体 | 全系支持 **32K** 上下文；更易接入 Hugging Face 与主流推理框架 | 主要仍是文本 Dense 模型；长上下文和复杂推理的稳定性仍待扩展 |
| **Qwen2** | 2024.06 | 0.5B / 1.5B / 7B / 57B-A14B / 72B | 扩展至 29 种语言；代码、数学提升；部分 Instruct 支持 **128K** | 128K 仅覆盖部分规格；通用模型的深度推理仍依赖后续专门训练 |
| **Qwen2.5** | 2024.09 | Dense：0.5B / 1.5B / 3B / 7B / 14B / 32B / 72B | 预训练规模至 18T token；强化代码／数学、结构化 JSON 输出和 8K 长输出 | 语言、代码、数学等能力仍由多个专门分支承载，统一多模态与推理尚未完全整合 |
| **Qwen3** | 2025.04 | Dense：0.6B–32B；MoE：30B-A3B / 235B-A22B | **Think / Non-Think 混合推理**；Dense 与 MoE 并行；36T token、119 种语言与方言 | 推理模式会带来额外延迟与 token 成本；MoE 版本的训练、部署和路由调优更复杂 |
...
![[Pasted image 20260810171244.png]]
...
![[Pasted image 20260810171303.png|600]]
...
![[Pasted image 20260810171332.png|600]]


Qwen3 系列的旗舰模型包含四阶段训练流程。==前两个阶段培养模型的思考能力，后两个阶段则将强大的非思考功能集成到模型中。==

> ==将教师模型的 logits 输出直接蒸馏到轻量级学生模型中，能够在保持对推理过程细粒度控制的同时有效提升其性能。==这种方法无需为每个小规模模型单独执行详尽的四阶段训练流程，不仅能通过更高的 Pass@1 分数，体现出更优的即时性能，还能通过改善的长序列探索能力增强模型的推理扩展性。另一方面，可以以更高的训练效率实现了这些提升，仅需四阶段训练方法 1/10 的 GPU 时长。
...
![[Pasted image 20260810171443.png|600]]
